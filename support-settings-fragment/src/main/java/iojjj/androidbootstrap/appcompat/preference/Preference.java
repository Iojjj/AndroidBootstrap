...
import iojjj.androidbootstrap.appcompat.internal.util.CharSequences;
import iojjj.androidbootstrap.appcompat.utils.SharedPreferencesHelper;

/**
 * Represents the basic Preference UI building
 * block displayed by a {@link android.preference.PreferenceActivity} in the form of a
 * {@link android.widget.ListView}. This class provides the {@link android.view.View} to be displayed in
 * the activity and associates with a {@link android.content.SharedPreferences} to
 * store/retrieve the preference data.
 * <p>
 * When specifying a preference hierarchy in XML, each element can point to a
 * subclass of {@link iojjj.androidbootstrap.appcompat.preference.Preference}, similar to the view hierarchy and layouts.
 * <p>
 * This class contains a {@code key} that will be used as the key into the
 * {@link android.content.SharedPreferences}. It is up to the subclass to decide how to store
 * the value.
 * 
...
public class Preference implements Comparable<Preference> {
    ...
    private int mLayoutResId = R.layout.au__preference;
     ...
     * {@link iojjj.androidbootstrap.appcompat.preference.Preference} has been changed by the user and is
    ...
    /**
     * Interface definition for a callback to be invoked when a {@link iojjj.androidbootstrap.appcompat.preference.Preference} is
     * clicked.
     */
     ...
     * {@link iojjj.androidbootstrap.appcompat.preference.Preference} is changed or, if this is a group, there is an
     * addition/removal of {@link iojjj.androidbootstrap.appcompat.preference.Preference}(s). This is used internally.
     */
    interface OnPreferenceChangeInternalListener {
        ...
        /**
         * Called when this group has added/removed {@link iojjj.androidbootstrap.appcompat.preference.Preference}(s).
         *
         * @param preference This Preference.
         */
    ...
    private static int getStyleableAttrId(int attr) {
        Log.w(TAG, String.format("Check against: %d", attr));
        return attr;
    }

    Preference(Context context, AttributeSet attrs, int defStyle, int defStyleId) {
        mContext = context;

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.Preference, defStyle, defStyleId);

        assert a != null;
        Log.w(TAG, String.format("Initializing: %s", ((Object) this).getClass().getSimpleName()));
        for (int i = a.getIndexCount() - 1; i >= 0; i--) {
            int attr = a.getIndex(i);
            Log.w(TAG, String.format("Attr is: %d", attr));

            if (attr == getStyleableAttrId(R.styleable.Preference_android_icon)) {
                mIconResId = a.getResourceId(attr, 0);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_key)) {
                mKey = a.getString(attr);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_title)) {
                mTitleRes = a.getResourceId(attr, 0);
                mTitle = a.getString(attr);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_summary)) {
                mSummary = a.getString(attr);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_order)) {
                mOrder = a.getInt(attr, mOrder);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_fragment)) {
                mFragment = a.getString(attr);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_layout)) {
                mLayoutResId = a.getResourceId(attr, mLayoutResId);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_widgetLayout)) {
                mWidgetLayoutResId = a.getResourceId(attr, mWidgetLayoutResId);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_enabled)) {
                mEnabled = a.getBoolean(attr, true);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_selectable)) {
                mSelectable = a.getBoolean(attr, true);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_persistent)) {
                mPersistent = a.getBoolean(attr, mPersistent);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_dependency)) {
                mDependencyKey = a.getString(attr);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_defaultValue)) {
                mDefaultValue = onGetDefaultValue(a, attr);

            } else if (attr == getStyleableAttrId(R.styleable.Preference_android_shouldDisableView)) {
        ...
        if (!clazz.getPackage().getName().endsWith("android.appcompat.preference")) {
            /*!clazz.getName().startsWith("android.preference")
        ...
        Log.w(TAG, String.format("Initialized: %s", this.toString()));
    }
    ...
    public Preference(Context context, AttributeSet attrs, int defStyle) {
        this(context, attrs, defStyle, R.style.Preference_DeviceDefault);
    }
    ...
    /**
     * Sets an {@link android.content.Intent} to be used for
     * {@link android.content.Context#startActivity(android.content.Intent)} when this Preference is clicked.
     * 
    ...
    /**
     * Return the {@link android.content.Intent} associated with this Preference.
     * 
     * @return The {@link android.content.Intent} last set via {@link #setIntent(android.content.Intent)} or XML.
     */
    ...
    /**
     * Sets the layout resource that is inflated as the {@link android.view.View} to be shown
     * for this Preference. In most cases, the default layout is sufficient for
     * custom Preference objects and only the widget layout needs to be changed.
     * <p>
     * This layout should contain a {@link android.view.ViewGroup} with ID
    ...
    /**
     * Gets the layout resource that will be shown as the {@link android.view.View} for this Preference.
     * 
     ...
     * @see #onCreateView(android.view.ViewGroup)
     ...
     * {@link android.preference.PreferenceGroup#setOrderingAsAdded(boolean)} can be used to order
     ...
     * storage.
     * 
     * @param persistent Set true if it should store its value(s) into the {@link android.content.SharedPreferences}.
     */
     ...
     * 
     * @param preferenceScreen A {@link android.preference.PreferenceScreen} whose hierarchy click
    ...
    /**
     * Returns an {@link android.content.SharedPreferences.Editor} where this Preference can
     ...
     * improve performance.
     * 
     * @return A {@link android.content.SharedPreferences.Editor} where this preference saves
    ...
    /**
     * Returns whether the {@link iojjj.androidbootstrap.appcompat.preference.Preference} should commit its saved value(s) in
     ...
     * {@link android.preference.PreferenceActivity}. This can also be called when this
     * Preference has been attached to a group that was already attached
     * to the {@link android.preference.PreferenceActivity}.
     */
     ...
     * even above/below your {@link android.preference.PreferenceScreen} screen break) with the given
     * key.
     * <p>
     ...
     * {@link iojjj.androidbootstrap.appcompat.preference.Preference#setDependency(String)} on the dependent Preference.
     * 
     ...
     * the {@link android.preference.PreferenceManager}, put in the string, and check if we should commit (and
     * commit if so).
     * 
    ...
    protected boolean persistStringSet(Set<String> values) {
        if (shouldPersist()) {
            ...
            SharedPreferences.Editor editor = mPreferenceManager.getEditor();
            SharedPreferencesHelper.putStringSet(editor, mKey, values);
            tryCommit(editor);
            return true;
        }
        return false;
     ...
     * @return The value from the SharedPreferences or the default return
     *         value.
     * @see #persistStringSet(java.util.Set)
     * 
     * @hide Pending API approval
     */
    protected Set<String> getPersistedStringSet(Set<String> defaultReturnValue) {
        ...
        return SharedPreferencesHelper.getStringSet(
                mPreferenceManager.getSharedPreferences(), mKey, defaultReturnValue);
    }
    ...
    /**
     * A base class for managing the instance state of a {@link iojjj.androidbootstrap.appcompat.preference.Preference}.
     */
