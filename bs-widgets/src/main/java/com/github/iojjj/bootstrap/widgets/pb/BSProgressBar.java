...
abstract class BSProgressBar extends View {
    ...
    public BSProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public BSProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(@NonNull Context context, @NonNull AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.BSFilledProgressBar);
        try {
            borderPaint.setStrokeWidth(array.getDimension(R.styleable.BSFilledProgressBar_fpb_borderStrokeWidth, 6));
            float textSize = array.getDimension(R.styleable.BSFilledProgressBar_fpb_textSize, 60);
            textPaint.setTextSize(textSize);
            textPaintWhite.setTextSize(textSize);
            borderPaint.setColor(array.getColor(R.styleable.BSFilledProgressBar_fpb_borderColor, ContextCompat.getColor(context, R.color.fpb_progress_border)));
            textPaintWhite.setColor(array.getColor(R.styleable.BSFilledProgressBar_fpb_textColorFilled, ContextCompat.getColor(context, R.color.fpb_progress_textFilled)));
            textPaint.setColor(array.getColor(R.styleable.BSFilledProgressBar_fpb_textColorEmpty, ContextCompat.getColor(context, R.color.fpb_progress_textEmpty)));
            progress = array.getInteger(R.styleable.BSFilledProgressBar_fpb_progress, 50);
            max = array.getInteger(R.styleable.BSFilledProgressBar_fpb_max, 100);
            int resourceId = array.getResourceId(R.styleable.BSFilledProgressBar_fpb_fillWith, 0);
            Drawable drawable = array.getDrawable(R.styleable.BSFilledProgressBar_fpb_fillWith);
            if (resourceId > 0) {
                if (drawable == null) {
